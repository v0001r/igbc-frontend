import {
  ...
  type EpiLimitCalculationLimitRow,
  type EpiLimitCalculationState,
} from "@/annexure/annexEpiLimitCalculationCalculations";
import {
  buildSavePayloadFromEpiLimitCalculation,
  hydrateEpiLimitCalculationAnnex,
} from "@/annexure/annexEpiLimitCalculationStorage";
...
function EpiLimitsTable({ row }: { row: EpiLimitCalculationLimitRow | null }) {
  if (!row) return null;
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[640px] border-collapse text-sm">
        <thead>
          <tr>
            <th className="border border-border bg-muted/30 px-3 py-2 text-left font-semibold">False</th>
            <th
              colSpan={5}
        ...
        <tbody>
          <tr>
            <td className="border border-border px-2 py-1.5">
              <TextInput value={row.bpo} />
            </td>
            <td className="border border-border px-2 py-1.5">
              <TextInput value={row.mandatory} />
            </td>
            <td className="border border-border px-2 py-1.5">
              <TextInput value={row.cp4} />
            </td>
            <td className="border border-border px-2 py-1.5">
              <TextInput value={row.cp6} />
            </td>
            <td className="border border-border px-2 py-1.5">
              <TextInput value={row.cp10} />
            </td>
            <td className="border border-border px-2 py-1.5">
              <TextInput value={row.cp14} />
            </td>
          </tr>
        </tbody>
      </table>
...
export function AnnexureEpiLimitCalculationRenderer({
  schema,
  tab,
  subtab,
  ratingTypeId,
  ...
  const [draft, setDraft] = useState<EpiLimitCalculationState>(() =>
    hydrateEpiLimitCalculationAnnex(schema, formState, tab, subtab),
  );

  useEffect(() => {
    setDraft(hydrateEpiLimitCalculationAnnex(schema, formState, tab, subtab));
  }, [schema, formState, tab, subtab, dataSignature]);

  const recalc = useCallback((fn: (s: EpiLimitCalculationState) => EpiLimitCalculationState) => {
    setDraft((prev) => computeEpiLimitCalculationState(fn(prev)));
  }, []);

  useEffect(() => {
    const handle: AnnexureRendererHandle = {
      getSaveFields: () => buildSavePayloadFromEpiLimitCalculation(draft),
    };
  ...
  const zoneRowIdx = climateZoneRowIndex(draft.cli_zone_annexone);
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-b-xl border border-border bg-card p-4 shadow-sm space-y-6">
          ...
          <table className="w-full max-w-xl border-collapse text-sm">
            <tbody>
              ...
              <tr>
                ...
                <td className="border border-border px-2 py-1.5">
                  <NumInput value={draft.percentage_air_cond_annexone} readOnly />
                </td>
              </tr>
              <tr>
        ...
        {visibleEpiRow ? (
          ...
          <p className="text-sm text-muted-foreground">Select a climate zone to view EPI limits.</p>
        )}
              ...
              onClick={() =>
                recalc((s) =>
                  s.energyRows.length >= maxEnergy
                    ...
                    : {
                        ...s,
                        energyRows: [...s.energyRows, { ...EPI_LIMIT_CALCULATION_DEFAULT_ENERGY_ROW }],
                      },
                )
              }
            >
